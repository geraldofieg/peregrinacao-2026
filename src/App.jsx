  ...
  return(
    <div style={{ padding:"20px 16px" }}>
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ ...T.sub, marginTop:4 }}>Endereço de cada hotel, além de atrações e lugares para comer por perto</div>
      </div>
      <Card style={{ background:"#E8F5E9", border:"1px solid #A5D6A7" }}>
        <div style={{ ...T.body, fontSize:16, lineHeight:1.7, color:C.green }}>
          🎯 Escolha "Atrações" ou "🍽️ Comida" para filtrar a lista — sempre ordenada do lugar mais perto para o mais longe do hotel. Toque num item para ver detalhes e depois em "Ver rota a pé".
        </div>
      </Card>
      <HotelListWithNearby />
    </div>
...
function HotelListWithNearby(){
  ...
  const [filtro, setFiltro] = useState({}); // { [hi]: "atracoes" | "comida" }

  return (
    <>
      {HOTELS.map((h, hi) => {
        const filtroAtual = filtro[hi] || "atracoes";
        const lista = filtroAtual === "atracoes" ? h.atracoes : h.comida;
        // Sempre ordenado do mais perto para o mais longe
        const listaOrdenada = [...(lista || [])].sort((a,b) => a.min - b.min);
        const temAmbos = (h.atracoes?.length > 0) || (h.comida?.length > 0);
            ...
            {temAmbos && (
              <>
                ...
                <div style={{ display:"flex", gap:8, marginTop:18, marginBottom:14 }}>
                  <button
                    onClick={() => setFiltro(prev => ({ ...prev, [hi]: "atracoes" }))}
                    style={{ flex:1, padding:"11px 8px", borderRadius:14, border:`2px solid ${filtroAtual==="atracoes"?C.gold:C.creamD}`, background:filtroAtual==="atracoes"?C.gold:C.white, color:filtroAtual==="atracoes"?C.white:C.brownM, fontFamily:"'Cinzel',serif", fontSize:14, fontWeight:700, cursor:"pointer" }}
                  >
                    🎯 Atrações {h.atracoes?.length > 0 ? `(${h.atracoes.length})` : ""}
                  </button>
                  <button
                    onClick={() => setFiltro(prev => ({ ...prev, [hi]: "comida" }))}
                    style={{ flex:1, padding:"11px 8px", borderRadius:14, border:`2px solid ${filtroAtual==="comida"?C.gold:C.creamD}`, background:filtroAtual==="comida"?C.gold:C.white, color:filtroAtual==="comida"?C.white:C.brownM, fontFamily:"'Cinzel',serif", fontSize:14, fontWeight:700, cursor:"pointer" }}
                  >
                    🍽️ Comida {h.comida?.length > 0 ? `(${h.comida.length})` : ""}
                  </button>
                </div>
                ...
                {listaOrdenada.length === 0 && (
                  <div style={{ background:C.creamD, borderRadius:10, padding:"14px", textAlign:"center", fontSize:14, color:C.brownM, fontStyle:"italic" }}>
                    Em breve — informações ainda sendo levantadas.
                  </div>
                )}
