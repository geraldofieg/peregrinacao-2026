...
const HOTELS = [
  {
    ...
    color: "#1A2B5F",
    nearby: [
      { icon:"⛪", nome:"Sé de Lisboa", tempo:"13 min", desc:"A Catedral de Lisboa, uma das construções religiosas mais antigas e emblemáticas da cidade, no histórico bairro de Alfama.", query:"Sé de Lisboa Catedral wikipedia" },
      { icon:"🏰", nome:"Castelo de São Jorge", tempo:"17 min", desc:"Uma das grandes atrações históricas de Lisboa, no alto de uma colina, com vistas panorâmicas sobre o centro histórico e o Rio Tejo.", query:"Castelo de São Jorge Lisboa Portugal wikipedia" },
      { icon:"🌊", nome:"Praça do Comércio", tempo:"14 min", desc:"Uma das praças mais emblemáticas de Lisboa, às margens do Tejo, cercada por arquitetura histórica, restaurantes e cafés.", query:"Praça do Comércio Lisboa Portugal wikipedia" },
      { icon:"🛗", nome:"Elevador de Santa Justa", tempo:"8 min", desc:"Um dos cartões-postais de Lisboa. Construído em ferro, liga a Baixa ao Largo do Carmo.", query:"Elevador de Santa Justa Lisboa Portugal wikipedia" },
      { icon:"🏰", nome:"Torre de Belém", tempo:"14 min", desc:"Símbolo da época dos Descobrimentos Portugueses, construída junto ao Rio Tejo. Um dos monumentos mais fotografados de Lisboa.", query:"Torre de Belém Lisboa Portugal wikipedia" },
      { icon:"🍴", nome:"Time Out Market Lisboa", tempo:"12 min", desc:"Um dos espaços gastronômicos mais famosos da cidade, reunindo restaurantes, chefs e sabores portugueses.", query:"Time Out Market Lisboa Portugal wikipedia" },
      { icon:"⛪", nome:"Mosteiro dos Jerónimos", tempo:"14 min", desc:"Magnífico monumento em estilo manuelino, um dos maiores símbolos da história portuguesa e da Era dos Descobrimentos.", query:"Mosteiro dos Jerónimos Lisboa Portugal wikipedia" },
      { icon:"🎨", nome:"MAAT — Museu de Arte, Arquitetura e Tecnologia", tempo:"20 min", desc:"Museu contemporâneo às margens do Tejo, conhecido pela arquitetura moderna e exposições de arte e tecnologia.", query:"MAAT Museu Lisboa Portugal wikipedia" },
      { icon:"🎭", nome:"LX Factory", tempo:"14 min", desc:"Antigo complexo industrial transformado em polo criativo, com restaurantes, lojas, arte urbana e livrarias.", query:"LX Factory Lisboa Portugal wikipedia" },
      { icon:"🚋", nome:"Elétrico 28", tempo:"11 min", desc:"O famoso elétrico amarelo percorre bairros históricos de Lisboa, uma das experiências mais tradicionais da cidade.", query:"Elétrico 28 Lisboa Portugal wikipedia" },
      { icon:"🎨", nome:"Museu Nacional do Azulejo", tempo:"17 min", desc:"Um passeio pela história dos tradicionais azulejos portugueses, instalado em um antigo convento do século XVI.", query:"Museu Nacional do Azulejo Lisboa Portugal wikipedia" },
      { icon:"🗿", nome:"Padrão dos Descobrimentos", tempo:"16 min", desc:"Monumento às margens do Tejo que homenageia figuras ligadas às grandes navegações portuguesas.", query:"Padrão dos Descobrimentos Lisboa Portugal wikipedia" },
      { icon:"🏛️", nome:"Arco Triunfal da Rua Augusta", tempo:"14 min", desc:"Um dos símbolos da Baixa de Lisboa, marcando a entrada da Rua Augusta com bela vista da Praça do Comércio.", query:"Arco da Rua Augusta Lisboa Portugal wikipedia" },
    ],
  },
  {
    city: "Roma 🇮🇹",
  ...
  return(
    <div style={{ padding:"20px 16px" }}>
      <div style={{ marginBottom:16 }}>
        <div style={T.pageTitle}>Hotéis</div>
        <div style={{ ...T.sub, marginTop:4 }}>Endereços, café da manhã incluso e atrações próximas de cada hospedagem</div>
      </div>
      <Card style={{ background:"#E8F5E9", border:"1px solid #A5D6A7" }}>
        <div style={{ ...T.body, fontSize:16, lineHeight:1.7, color:C.green }}>
          🚶 Toque em uma atração para ver a descrição, e use "Ver rota" para abrir o caminho a pé no Google Maps, partindo de onde você estiver.
        </div>
      </Card>
      <HotelListWithNearby />
    </div>
      ...
      {HOTELS.map((h, hi) => (
        <Card key={hi}>
          ...
          {h.nearby && h.nearby.length > 0 && (
            <>
              <div style={{ ...T.label, marginTop:18, marginBottom:10, display:"block" }}>📌 Atrações próximas ao hotel</div>
              {h.nearby.map((n, ni) => {
                const key = `${hi}-${ni}`;
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", background:open?C.creamD:C.white }}>
                      <div style={{ display:"flex", gap:10, alignItems:"center", flex:1 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize:13, color:C.brownM, marginTop:2 }}>🚶 {n.tempo} a pé</div>
                        </div>
                      </div>
                    ...
                    {open && (
                      <div style={{ padding:"12px 14px", background:"#FFFDF8", borderTop:`1px solid ${C.creamD}` }}>
                        ...
                        <div style={{ display:"flex", gap:8 }}>
                          ...
                          <button
                            onClick={e=>{ e.stopPropagation(); abrirRota(n.nome + " " + (h.city||"")); }}
